...
router.get('/', function(req, res, next) {
    connection.query('select count(*) as total from articles', function(error, results) {
        if (error) {
            ...
            connection.query('select a.*,t.title as typename from articles a,types t  where t.id=a.tid order by a.id desc limit ' + (page.now - 1) * page.every + ',' + page.every, function(error, results) {
                if (error) {
                    console.log('查询数据失败', error);
                } else {
                    connection.query('select * from types', function(error, results1) {
                        if (error) {
                            ...
                            res.render('articles/index', { results: results, moment: require('moment'), page: page, admin: req.session.admin, results1: results1 });
                        }
                    })
                }
            })
...
router.post('/ainsert', function(req, res) {
    ...
    connection.query('insert into articles(tid,title,content,addtime) values(' + req.body.tid + ',"' + req.body.title + '","' + req.body.content + '",' + req.body.addtime + ')', function(error, results) {
        if (error) {
            ...
            connection.query('select t.title as typename,a.* from articles a,types t where t.id=a.tid and a.id = ' + results.insertId, function(error, results1) {
                if (error) {
                    console.log('查询失败', error);
                } else {
                    var data = results1[0];
...
router.post('/update', function(req, res) {
    ...
    connection.query('update articles set title="' + req.body.title + '",content="' + req.body.content + '" where id=' + req.body.id, function(error, results) {
        if (error) {
