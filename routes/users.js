...
router.get('/', function(req, res, next) {
    connection.query('select count(*) as total from users', function(error, results) {
        if (error) {
            ...
            connection.query('select * from users order by users.id desc limit ' + (page.now - 1) * page.every + ',' + page.every, function(error, results) {
                if (error) {
...
router.get('/insert', function(req, res) {
    res.render('users/insert', { admin: req.session.admin });
});
...
router.get('/update/:id', function(req, res) {
    connection.query('select * from users where id=' + req.params.id, function(error, results) {
        if (error) {
            ...
            console.log(results);
            res.render('users/update', { results: results[0], admin: req.session.admin });
        }
    })
});
